...
WITH token_stats AS (
    ...
    FROM {{ ref('stg_recent_transactions') }}
...
latest_followers AS (
    ...
    FROM {{ source('bronze', 'x_followers') }}
...
enriched_tokens AS (
    ...
    LEFT JOIN {{ source('bronze', 'token_metadata') }} tm 
        ON st.token_address = tm.token_address
    LEFT JOIN {{ source('bronze', 'token_creation_info') }} tci 
        ON st.token_address = tci.token_address
    LEFT JOIN {{ source('bronze', 'solsniffer_token_data') }} ss 
