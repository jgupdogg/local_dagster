...
# Helper function to create an asset for each config
def create_gsheets_asset(config: Dict):
    ...
    def _asset(context: AssetExecutionContext):
        ...
        if not worksheet_name:
            raise ValueError(f"Worksheet name not found in environment variable {config['worksheet_env_var']}")
        
        context.log.info(f"Exporting data from {config['table']} to worksheet {worksheet_name}")
        
        # Query the database
        query = f"SELECT * FROM {config['table']} ORDER BY {config['order_by']} LIMIT {config['limit']}"
        ...
        try:
            ...
            context.add_output_metadata({
                ...
                "updated_cells": result.get('updated_rows', 0) * result.get('updated_columns', 0)
            })
